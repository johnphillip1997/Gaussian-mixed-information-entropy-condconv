    ...
    def forward_single(self, x, policy):
        ...
        for segment, num_blocks in enumerate(self.layer_config):
            for b in range(num_blocks):
                residual = x
                if policy[t] == 1:
                    y = self.blocks[segment][b](x)
                    x = residual + y
                x = F.relu(x)
                t += 1
                x = self.dc[segment][b](x)
            if segment != len(self.layer_config) - 1:
                x = self.maxpool(x)
        #  x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
...
class FlatResNet32(FlatResNet):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        self.inplanes = 4
        self.conv1 = base.conv3x3(3, 4)
        self.bn1 = nn.BatchNorm2d(4)
        self.relu = nn.ReLU(inplace=True)
      #  self.avgpool = nn.AvgPool2d(8)
        self.maxpool=nn.MaxPool2d(2)
        strides = [1, 1, 1,1]
        filt_sizes = [4, 8, 16,32]
        filt_sizess = [4, 8, 16,32,32]
        self.filt_size=filt_sizes
        self.blocks, self.dc = [], []
        for idx, (filt_size, num_blocks, stride) in enumerate(zip(filt_sizes, layers, strides)):
            ...
            for i in range(num_blocks):
                if i==num_blocks-1:
                    dcn=nn.Conv2d(filt_sizess[idx],filt_sizess[idx+1],kernel_size=1)
                else:
                    dcn = nn.Conv2d(filt_sizess[idx], filt_sizess[idx], kernel_size=1)
                dc.append(dcn)
            self.dc.append(nn.ModuleList(dc))
        self.blocks = nn.ModuleList(self.blocks)
        self.dc = nn.ModuleList(self.dc)
    ...
    def _make_layer(self, block, planes, blocks, stride=1,idx=0):
        ...
        layers = [block(self.filt_size[idx], self.filt_size[idx], 1)]

        for i in range(1, blocks):
            layers.append(block(self.filt_size[idx], self.filt_size[idx], 1))
