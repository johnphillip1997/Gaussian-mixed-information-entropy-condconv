...
from utils import get_model,LrScheduler,performance_stats
...
parser.add_argument('--cv_dir', default='*', help='checkpoint directory (models and logs are saved here)')
...
def get_reward(preds, targets, policy):
    ...
    reward[match==False] = args.penalty
...
def train(epoch):
    ...
    accuracy, reward, sparsity, variance, policy_set = performance_stats(policies, rewards, matches)
...
def test(epoch):
 ...
 #   log_value('test_reward', reward, epoch)
  ...
  #  log_value('test_unique_policies', len(policy_set), epoch)
    ...
    rent_state_dict = rnet.module.state_dict() if args.parallel else agent.state_dict()
    state = {
      'agent': agent_state_dict,
      'resnet':rent_state_dict,
      'epoch': epoch,
      'reward': reward,
      'acc': accuracy
    }
...
#use own dataset shold do it yourself like below:
#train_dataset = datasets.ImageFolder(root=***,
 #                                    transform=data_transform["train"])
#trainset = torch.utils.data.DataLoader(train_dataset,
#                                         batch_size=batch_size, shuffle=True,
#                                        num_workers=0)
trainset, testset = utils.get_dataset(args.model, args.data_dir)

trainloader = torchdata.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testloader = torchdata.DataLoader(testset, batch_size=args.batch_size, shuffle=False)
rnet, agent = get_model(args.model)
num_blocks = 6
start_epoch = 0
